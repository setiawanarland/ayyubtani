        ...
        <div class="offset-content tab-content">
            <div id="activity" class="tab-pane fade in show active">
                <div class="recent-activity">
                    ...
                    <form id="kurangStokForm" data-type="submit">
                        @csrf
                        ...
                        <div class="form-group" style="margin-bottom: 0px;">
                            <label for="stok_kurang" class="col-form-label">Stok Kurang</label>
                            <input class="form-control" type="number" name="stok_kurang" id="stok_kurang">
                            <div class="invalid-feedback"></div>
                        </div>
                        <div class="form-group" style="margin-bottom: 0px;">
...
@section('scripts')
    <script>
        ...
        var dataRow = function() {
            var init = function() {
                let table = $('#kurangStokTable');
                table.DataTable({
                    processing: true,
                    ordering: false,
                    ajax: "{{ route('list-kurang-stok') }}",
                    columns: [{
                            data: 'nama_produk',
            ...
            var destroy = function() {
                var table = $('#kurangStokTable').DataTable();
                table.destroy();
            };
        ...
        // calculate stok
        $(document).on('keyup', '#stok_kurang', function() {
            let stokKurang = $(this).val();
            let stok = $('#stok').val();
            let qtyDos = $('#qty_dos').val();

            let stokAfter = parseInt(stok) - parseInt(stokKurang);
            $('#stok_after').val(stokAfter);
        });

        // process tambah stok
        $(document).on('submit', "#kurangStokForm[data-type='submit']", function(e) {
            e.preventDefault();

            var form = document.querySelector('form');
            var formData = new FormData(this);

            AxiosCall.post("{{ route('process-kurang') }}", formData,
                "#kurangStokForm");
        });
